...
const bloom = new UnrealBloomPass(new THREE.Vector2(innerWidth, innerHeight), 0.13, 0.30, 0.76);
...
function wrapText(ctx, text, x, y, maxW, lineH) {
  ...
  words.forEach((w, i) => {
    ...
    if (ctx.measureText(test).width > maxW && i > 0) {
      ctx.fillText(line.trimEnd(), x, cy); line = w + " "; cy += lineH;
    } else line = test;
  });
...
function makePlanetTex(col, variant, size = 512) {
  ...
  // Spherical base — offset light source for 3D depth
  ...
  if (variant === 0) {
    // CULTURALVERSE — earthy terrain with landmasses
    for (let i = 0; i < 8; i++) {
      ctx.save(); ctx.translate(rng()*size, rng()*size); ctx.rotate(rng()*Math.PI);
      ctx.beginPath();
    ...
    for (let i = 0; i < 5; i++) {
      ctx.beginPath(); ctx.arc(rng()*size, rng()*size, 8+rng()*30, 0, Math.PI*2);
      ctx.fillStyle = `rgba(${lt(R,58)},${lt(G,42)},${lt(B,18)},0.14)`; ctx.fill();
    }
    ...
    ctx.fillStyle = haze; ctx.fillRect(0, 0, size, size);

  } else if (variant === 1) {
    // LIVING KNOWLEDGE — flowing organic bands
    for (let i = 0; i < 6; i++) {
      const y0 = rng()*size;
      ...
      ctx.strokeStyle = `rgba(${lt(R,28)},${lt(G,52)},${lt(B,24)},0.13)`; ctx.stroke();
    }
    for (let i = 0; i < 22; i++) {
      ctx.beginPath(); ctx.arc(rng()*size, rng()*size, 2+rng()*8, 0, Math.PI*2);
      ctx.fillStyle = `rgba(255,255,255,${0.06+rng()*0.12})`; ctx.fill();
    }

  } else if (variant === 2) {
    // IKESTAR — icy crystal + star pinpoints + polar cap
    for (let i = 0; i < 18; i++) {
      ctx.beginPath(); ctx.moveTo(rng()*size, rng()*size); ctx.lineTo(rng()*size, rng()*size);
      ctx.strokeStyle = `rgba(255,255,255,${0.06+rng()*0.12})`; ctx.lineWidth = 0.5+rng()*1.5; ctx.stroke();
    }
    for (let i = 0; i < 42; i++) {
      ctx.beginPath(); ctx.arc(rng()*size, rng()*size, 0.8+rng()*2.4, 0, Math.PI*2);
    ...
    cap.addColorStop(0, `rgba(255,255,255,0.6)`); cap.addColorStop(1, `rgba(255,255,255,0)`);
    ctx.fillStyle = cap; ctx.fillRect(0, 0, size, size);

  } else {
    // COSMIC WEAVE — nebula swirls
    for (let i = 0; i < 6; i++) {
      const cx2 = size*(0.18+rng()*0.64), cy2 = size*(0.18+rng()*0.64);
    ...
    for (let i = 0; i < 26; i++) {
      ctx.beginPath(); ctx.arc(rng()*size, rng()*size, 0.8+rng()*2.2, 0, Math.PI*2);
      ctx.fillStyle = `rgba(255,255,255,${0.28+rng()*0.72})`; ctx.fill();
    }
  }
  ...
  spec.addColorStop(0, `rgba(255,255,255,0.24)`); spec.addColorStop(1, `rgba(255,255,255,0)`);
...
// ─────────────────────────────────────────────────────────────────────────────
// ORBIT TEXT RING — billboard disc with circular ticker crawl text
// The disc always faces the camera (via textBillboard.lookAt); textSpin.rotation.z
// increments each frame for the crawl effect.
// Characters are drawn tangent to their arc position; the top of the disc is
// always readable since the billboard keeps the disc facing the viewer.
// ─────────────────────────────────────────────────────────────────────────────
function makeOrbitTextTex(text, accentHex, size = 1024) {
  ...
  // Repeat text with separator to fill full circle
  const sep       = "  ·  ";
  const unit      = text + sep;
  const charPx    = size * 0.036; // approx char width
...
function makeCardInfoTex(app) {
  ...
  ctx.fillStyle = hex; ctx.fillRect(0, 0, W, 5);
  ctx.fillStyle = "rgba(212,174,90,0.9)";
  ctx.font = "600 24px 'DM Sans', sans-serif"; ctx.textAlign = "left"; ctx.textBaseline = "top";
  ctx.fillText("IKEVERSE APP", 40, 32);

  ctx.fillStyle = "rgba(228,241,255,0.98)";
  ctx.font = "800 52px 'DM Sans', sans-serif"; ctx.textBaseline = "alphabetic";
  ...
  ctx.fillStyle = hex; ctx.font = "700 21px 'DM Sans', sans-serif";
  ctx.textAlign = "left"; ctx.textBaseline = "middle"; ctx.fillText(app.status.toUpperCase(), 58, 229);
...
function makeHubTitleTex(text) {
  ...
  ctx.shadowColor = "rgba(84,198,238,0.65)"; ctx.shadowBlur = 30;
  ctx.fillStyle   = "rgba(212,174,90,0.97)";
  ctx.font        = "900 76px 'Orbitron', monospace";
  ctx.textAlign   = "center"; ctx.textBaseline = "middle";
  ctx.fillText(text, W/2, H/2);

  ctx.shadowBlur = 0; ctx.fillStyle = "rgba(84,198,238,0.48)";
...
function makeLights() {
  ...
  const key = new THREE.DirectionalLight(0xf4dea8, 1.52); key.position.set(7, 12, 9); scene.add(key);
  const rim  = new THREE.PointLight(0x7a60ff, 13, 44, 2); rim.position.set(-9, 7, -10); scene.add(rim);
  const fill = new THREE.PointLight(0x44b4ee, 15, 36, 2); fill.position.set(9, 3, 11); scene.add(fill);
  const hf   = new THREE.PointLight(0x55d4ff, 5, 11, 2); hf.position.set(0, 4, 0); scene.add(hf);
  state.hub.fillLight = hf;
}

// ─────────────────────────────────────────────────────────────────────────────
// BACKGROUND — two-layer stars + galaxy + nebulas
// ─────────────────────────────────────────────────────────────────────────────
function addStars(count, minR, maxR, size, opacity, colors) {
  const pos = new Float32Array(count * 3), col = new Float32Array(count * 3);
  for (let i = 0; i < count; i++) {
    const r = minR + Math.random() * (maxR - minR);
    const θ = Math.random() * Math.PI * 2, φ = Math.acos(Math.random() * 2 - 1);
    pos[i*3]   = r * Math.sin(φ) * Math.cos(θ);
    pos[i*3+1] = r * Math.cos(φ) * 0.70;
  ...
  bgGroup.add(new THREE.Points(geo, new THREE.PointsMaterial({
    size, transparent: true, opacity, depthWrite: false,
...
function buildGalaxy(starCount, arms, spreadFactor, maxR) {
  const pos = new Float32Array(starCount * 3), col = new Float32Array(starCount * 3);
...
function makeBackground() {
  // Distant blue-white stars
  addStars(2800, 90, 170, 0.065, 0.88, [
    new THREE.Color(0x9ed8ff), new THREE.Color(0xffffff), new THREE.Color(0xc2e4ff)
  ]);
  // Mid-field warm/purple stars
  addStars(1000, 52, 95, 0.105, 0.65, [
    new THREE.Color(0xf6e8cc), new THREE.Color(0xb48cff), new THREE.Color(0x88ccff)
  ]);
  ...
  // ── SECOND DISTANT GALAXY (elliptical) ─────────────────────────────────────
  ...
  [
    ...
    spr.position.set(x, y, z); spr.scale.set(w, h, 1); bgGroup.add(spr);
  });
  ...
  [
    ...
    spr.position.set(...pos); spr.scale.set(w, h, 1); bgGroup.add(spr);
  });
}

// ─────────────────────────────────────────────────────────────────────────────
// ENVIRONMENT — floor, orbit rings, dust
// ─────────────────────────────────────────────────────────────────────────────
function makeEnv() {
  const floor = new THREE.Mesh(
    new THREE.CircleGeometry(12.5, 80),
    new THREE.MeshBasicMaterial({ color: 0x0b172e, transparent: true, opacity: 0.44, blending: THREE.AdditiveBlending, depthWrite: false })
  );
  ...
  [5.8, 8.6, 11.2].forEach((r, i) => {
    const ring = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: i === 1 ? 0x283e7a : 0x192e58, transparent: true, opacity: i === 1 ? 0.17 : 0.11 })
    );
  ...
  for (let i = 0; i < dustCount; i++) {
    const a = Math.random() * Math.PI * 2, r = 5.0 + Math.random() * 5.2;
    dPos[i*3] = Math.cos(a)*r; dPos[i*3+1] = -0.2 + (Math.random()-0.5)*0.13; dPos[i*3+2] = Math.sin(a)*r;
  }
  const dGeo = new THREE.BufferGeometry();
  dGeo.setAttribute("position", new THREE.BufferAttribute(dPos, 3));
  const dust = new THREE.Points(dGeo, new THREE.PointsMaterial({
    color: 0x52b4ff, size: 0.030, opacity: 0.17, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
  }));
  envGroup.userData.dust = dust; envGroup.add(dust);
}

// ─────────────────────────────────────────────────────────────────────────────
// HUB — sphere (planet texture + icon decal) + 3 rings + Orbitron title
// ─────────────────────────────────────────────────────────────────────────────
function makeHub() {
  ...
  // Hub sphere — dark base material
  const hubCol = new THREE.Color(0x4488bb);
  const hubPlanetTex = makePlanetTex(hubCol, 4, 512); // variant 4 → cosmic swirl variant
  ...
  // Icon decal — wrapped on top of planet texture
  const hubDecal = new THREE.Mesh(
    new THREE.SphereGeometry(1.255, 48, 48),
    new THREE.MeshBasicMaterial({
      map: hubTex || null,
      transparent: true, opacity: hubTex ? 0.88 : 0,
      depthWrite: false
    })
  );
  ...
  ringA.position.y = 1.8; ringA.rotation.x = Math.PI / 2;
  ...
  ringB.position.y = 1.8; ringB.rotation.x = Math.PI / 2; ringB.rotation.z = (32*Math.PI)/180;

  // Ring C — -38° tilt (violet)
  ...
  ringC.position.y = 1.8; ringC.rotation.x = Math.PI / 2; ringC.rotation.z = (-38*Math.PI)/180;

  const glow = new THREE.Sprite(new THREE.SpriteMaterial({
    map: makeRadialTex(84, 198, 238, 0.14, 256),
    transparent: true, opacity: 1, depthWrite: false, blending: THREE.AdditiveBlending
  }));
  glow.position.y = 1.8; glow.scale.set(4.8, 4.8, 1);

  const title = new THREE.Sprite(new THREE.SpriteMaterial({
    map: makeHubTitleTex(HUB_NAME), transparent: true, opacity: 0.95, depthWrite: false
  }));
  title.position.set(0, 3.58, 0); title.scale.set(3.50, 0.64, 1);

  grp.add(base, glow, ringA, ringB, ringC, sphere, hubDecal, title);
  world.add(grp);
  Object.assign(state.hub, { group: grp, base, sphere, hubDecal, ringA, ringB, ringC, glow, title });
}

// ─────────────────────────────────────────────────────────────────────────────
// APP NODES
//
// Node structure (per app):
//   group
//     ├─ ped              pedestal cylinder
//     ├─ track            orbit track torus
//     ├─ selRing          selection ring (floor, fades in)
//     ├─ orbPivot         bobs vertically
//     │    ├─ orb         planet sphere (procedural texture)
...
//     └─ cardGrp          opacity-only reveal card (NO bob)
// ─────────────────────────────────────────────────────────────────────────────
function makeNodes() {
  APPS.forEach((app, idx) => {
    ...
    track.rotation.x = Math.PI / 2; track.position.y = 1.34;

    // ── Selection ring
    const selRing = new THREE.Mesh(
      new THREE.TorusGeometry(1.08, 0.010, 14, 120),
      new THREE.MeshBasicMaterial({ color: app.color, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false })
    );
    ...
    // App icon decal — slightly larger sphere, wraps icon on top of planet
    ...
    // ── Text ring — billboard disc with ticker crawl
    // textBillboard.lookAt(camera) every frame — keeps disc facing viewer
    // textSpin.rotation.z decrements for crawl
    ...
    // ── Card group — sibling of orbPivot, no bob
    ...
    const cardBg = new THREE.Mesh(
      ...
      new THREE.MeshPhysicalMaterial({ color: 0x060c1a, transparent: true, opacity: 0, roughness: 0.18, metalness: 0.02 })
    );
    ...
    const cardImg = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: appTex ? 0xffffff : app.color, map: appTex || null, transparent: true, opacity: 0 })
    );
    cardImg.position.y = 0.24;

    const cardInfo = new THREE.Mesh(
      new THREE.PlaneGeometry(2.32, 0.60),
      new THREE.MeshBasicMaterial({ map: makeCardInfoTex(app), transparent: true, opacity: 0 })
    );
    ...
    state.nodes.set(app.id, {
      ...
      cardGrp, cardBg, cardBorder, cardImg, cardInfo,
      accentColor: col
    });
    nodeRoot.add(group);
  });
...
function makeConnections() {
  while (connGrp.children.length) {
    const c = connGrp.children[0]; c.geometry?.dispose(); c.material?.dispose(); connGrp.remove(c);
  }
  const hub = new THREE.Vector3(0, 1.8, 0);
  APPS.forEach((app) => {
    const nd = state.nodes.get(app.id); if (!nd) return;
    const end = nd.group.position.clone(); end.y = 1.34;
    ...
    const mat = new THREE.LineDashedMaterial({ color: app.color, transparent: true, opacity: 0.17, dashSize: 0.16, gapSize: 0.10 });
...
// ─────────────────────────────────────────────────────────────────────────────
function buildDock() {
  dockEl.innerHTML = ""; state.dockBtns.clear();
  const ov = document.createElement("button");
  ov.type = "button"; ov.className = "dock-btn"; ov.textContent = "Overview";
  ov.addEventListener("click", () => resetView(true)); dockEl.appendChild(ov);
  state.dockBtns.set("overview", ov);
  APPS.forEach((app, i) => {
    const btn = document.createElement("button");
    btn.type = "button"; btn.className = "dock-btn"; btn.textContent = `${i+1}. ${app.title}`;
    btn.addEventListener("click", () => selectApp(app.id, true));
    dockEl.appendChild(btn); state.dockBtns.set(app.id, btn);
  });
  syncDock();
}
...
function syncDock() {
  state.dockBtns.forEach((btn, key) => {
    btn.classList.toggle("active", state.selected === null ? key === "overview" : key === state.selected);
  });
}

function setPanel(app) {
  if (!app) {
    elTitle.textContent = "IkeHub"; elSubtitle.textContent = "The gateway into the Ikeverse ecosystem.";
    elDesc.textContent = "Select an app portal to focus the camera, inspect the experience, and launch.";
    elStatus.textContent = "Showcase Portal"; elDot.style.color = "#54c6ee";
    elLaunch.href = "#"; elLaunch.textContent = "Select a Portal";
    elTags.innerHTML = ""; ["Three.js","Portal Hub","App Showcase","Ikeverse"].forEach(addTag);
    elMode.textContent = "Overview";
  } else {
    elTitle.textContent = app.title; elSubtitle.textContent = app.subtitle;
    elDesc.textContent = app.description; elStatus.textContent = app.status;
    elDot.style.color = "#" + new THREE.Color(app.color).getHexString();
    elLaunch.href = app.href; elLaunch.textContent = `Launch ${app.title}`;
    elTags.innerHTML = ""; app.tags.forEach(addTag);
    elMode.textContent = "Focused"; liveEl.textContent = `${app.title} selected`;
  }
}
function addTag(tag) { const s = document.createElement("span"); s.className = "tag"; s.textContent = tag; elTags.appendChild(s); }

// ─────────────────────────────────────────────────────────────────────────────
// CAMERA
// ─────────────────────────────────────────────────────────────────────────────
function resetView(userInput = false) {
  state.selected = null; state.spinning = true;
  ...
  setPanel(null); syncDock();
  if (userInput) markInput();
}

function selectApp(appId, userInput = false) {
  const nd = state.nodes.get(appId); if (!nd) return;
  state.selected = appId; state.spinning = false;

  // Use world position — accounts for nodeRoot idle-spin rotation
...
function markInput() { state.lastInput = performance.now(); }
...
function attachEvents() {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "Escape") { resetView(true); return; }
    const n = Number(e.key);
    if (Number.isInteger(n) && n >= 1 && n <= APPS.length) selectApp(APPS[n-1].id, true);
  });
...
function onResize() {
  ...
  placeNodes(); makeConnections();
  if (state.selected) selectApp(state.selected, false); else resetView(false);
}
...
function updateHub() {
  const h = state.hub; if (!h.group) return;
...
function updateNodes(t) {
  APPS.forEach((app, idx) => {
    ...
    // Bob orbPivot (orb + decal + atm + textBillboard all bob)
    ...
    nd.selRing.material.opacity = THREE.MathUtils.lerp(nd.selRing.material.opacity, sel ? 0.34 : 0, 0.074);

    // Orb emissive
    nd.orb.material.emissiveIntensity = THREE.MathUtils.lerp(nd.orb.material.emissiveIntensity, sel ? 0.42 : hov ? 0.32 : 0.18, 0.1);

    // Atmospheric halo
    nd.atm.material.opacity = THREE.MathUtils.lerp(nd.atm.material.opacity, sel ? 0.58 : hov ? 0.48 : 0.32, 0.1);

    // Track opacity
    nd.track.material.opacity = THREE.MathUtils.lerp(nd.track.material.opacity, sel ? 0.17 : hov ? 0.13 : 0.09, 0.1);

    // TEXT BILLBOARD — always face camera; textSpin rotates for ticker crawl
    nd.textBillboard.lookAt(camera.position);
    const crawl = sel ? 0.010 : hov ? 0.006 : 0.0022;
    nd.textSpin.rotation.z -= crawl;
    nd.textRing.material.opacity = THREE.MathUtils.lerp(nd.textRing.material.opacity, sel ? 0.0 : hov ? 1.0 : 0.76, 0.1);

    // Card — opacity-only reveal, zero scale change
...
function updateBackground(t) {
  ...
  if (envGroup.userData.dust) envGroup.userData.dust.rotation.z += 0.00024;
}
...
function onFrame() {
  ...
  if (state.spinning) { nodeRoot.rotation.y += 0.00095; connGrp.rotation.y += 0.00095; }

  updateHover(); updateHub(); updateNodes(t); updateBackground(t); updateCamera();
  composer.render();
}

// ─────────────────────────────────────────────────────────────────────────────
// INIT — after fonts load so Orbitron is available in canvas
// ─────────────────────────────────────────────────────────────────────────────
